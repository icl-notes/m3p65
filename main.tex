...
Thus the disjunctive normal form is
$$ \orb{\andb{p_1}{\notb{p_2}}}{\andb{\notb{p_1}}{\notb{p_2}}}. $$
\end{example*}
...
\hfill
\begin{enumerate}
\item If $ \phi $ is built using $ \lor, \land $, then $ F_\phi\br{T, \dots, T} = T $, by proof by induction on the number of connectives.
...
\end{itemize}
\item A \textbf{proof} in $ \Sigma $ is a finite sequence of formulas in $ \FF $, $ \phi_1, \dots, \phi_n $ such that each $ \phi_i $ is either an axiom, that is it is in $ \AA $, or is obtained from $ \phi_1, \dots, \phi_{i - 1} $ using one of the deduction rules.
...
\label{thm:1.2.5}
Suppose $ \Gamma $ is a set of $ L $-formulas and $ \phi $ and $ \psi $ are $ L $-formulas. Suppose $ \Gamma \cup \cbr{\phi} \vdash_L \psi $. Then $ \Gamma \vdash_L \impb{\phi}{\psi} $.
\end{theorem}
...
\end{enumerate}
In case $ 1 $, we argue as in the base case to get $ \Gamma \vdash_L \impb{\phi}{\psi} $. In case $ 2 $, there are formulas $ \chi $ and $ \impb{\chi}{\psi} $ earlier in the deduction. We use the inductive hypothesis to get
\begin{equation}
\label{eq:1}
...
\item MP preserves tautologies, that is if $ \psi $ and $ \impb{\psi}{\chi} $ are tautologies, so is $ \chi $.
\end{enumerate}
\begin{enumerate}
...
Suppose $ \Gamma $ is a set of formulas and $ \phi $ is a formula with $ \Gamma \vdash_L \phi $. Suppose $ v $ is a valuation with $ v\br{\psi} = T $ for all $ \psi \in \Gamma $. Then $ v\br{\phi} = T $.
\end{theorem}

\begin{proof}
Exercise: the same proof.
\end{proof}
...
& \implies \qquad \Gamma^* \not\vdash_L \psi & \text{by the inductive hypothesis} \\
...
\Gamma^* \not\vdash_L \notb{\psi},
\end{equation}
as $ \vdash_L \impb{\notb{\psi}}{\impb{\psi}{\chi}} $ by \ref{prop:1.2.7}.$ 1 $. By $ \br{\ref{eq:10}} $, $ \br{\ref{eq:11}} $, and the inductive hypothesis,
...
\begin{proof}
Exercise: deduce these from the preliminaries to the completeness theorem. A warning is that, in $ 2 $, do not assume that $ \Delta $ is consistent.
\end{proof}
...
Use the compactness theorem to prove $ P $ is compact.
\end{example*}
...
\hfill
\begin{itemize}
\item The ordering $ \le $ on $ \R $ is a binary relation on $ \R $.
\item $ + $ on $ \C $ is a binary function on $ \C $.
\item The even integers as a subset of $ \Z $ is a unary relation on $ \Z $.
\end{itemize}
\end{example*}
...
The information
$$ \br{n_i \mid i \in I}, \qquad \br{m_j \mid j \in J}, \qquad K $$
is called the \textbf{signature} of $ \LL $.
\end{definition}
...
\item If $ \phi $ and $ \psi $ are $ \LL $-formulas, then
...
\begin{example*}
Suppose $ \LL $ has a binary function symbol $ f $, a unary relation symbol $ P $, a binary relation symbol $ R $, and constant symbols $ c_1 $ and $ c_2 $.
\begin{itemize}
\item Some terms.
...
\begin{definition}
Suppose $ \phi $ and $ \psi $ are $ \LL $-formulas.
\begin{itemize}
\item $ \teb{x}\phi $ means $ \notb{\fab{x}\notb{\phi}} $.
...
\end{itemize}
then $ v $ is a well-defined valuation. (Exercise)
\end{proof}
...
\begin{definition}
Suppose $ \AA $ is an $ \LL $-structure and $ x_l $ is any variable. Suppose $ v $ and $ w $ are valuations in $ \AA $. We say $ v $ and $ w $ are \textbf{$ x_l $-equivalent} if $ v\br{x_m} = w\br{x_m} $ whenever $ m \ne l $.
\end{definition}
...
\item Suppose $ \phi $ and $ \psi $ are $ \LL $-formulas, and we already know about valuations satisfying $ \phi $ and $ \psi $.
\begin{align*}
...
Suppose $ \LL $ is a first-order language and $ \phi_1 $ and $ \phi_2 $ are $ \LL $-formulas. Substitute $ \phi_1 $ in place of $ p_1 $, and $ \phi_2 $ in place of $ p_2 $, in $ \chi $. We obtain an $ \LL $-formula
...
$$ \chi : \impb{\alpha}{\beta}. $$
So $ \theta $ is $ \impb{\theta_1}{\theta_2} $, where $ \theta_1 $ is obtained from $ \alpha $ and $ \theta_2 $ is obtained from $ \beta $. By the induction hypothesis,
$$ w\br{\alpha} = v\sbr{\theta_1}, \qquad w\br{\beta} = v\sbr{\theta_2}. $$
So $ w\br{\impb{\alpha}{\beta}} = v\sbr{\impb{\theta_1}{\theta_2}} $, etc.
\end{proof}
...
\begin{definition}
Suppose $ \phi $ and $ \psi $ are $ \LL $-formulas and $ \fab{x_i}\phi $ occurs as a subformula of $ \psi $, that is $ \psi $ is
...
\begin{theorem}
\label{thm:2.3.3}
Suppose $ \phi $ is a closed $ \LL $-formula and $ \AA $ is an $ \LL $-structure. Then either $ \AA \vDash \phi $ or $ \AA \vDash \notb{\phi} $. More generally, if $ \phi $ has free variables amongst $ x_1, \dots, x_n $, and $ v $ and $ w $ are valuations in $ \AA $ with
...
\begin{proof}
Note that the first statement follows from the generalisation, since if $ \phi $ has no free variables, then, for any valuations $ v $ and $ w $ in $ \AA $, they agree on the free variables of $ \phi $, so
$$ v\sbr{\phi} = w\sbr{\phi}. $$
...
to mean $ v\sbr{\psi} = T $ for every valuation $ v $ in $ \AA $ with
...
\begin{example*}
A warning example, where $ \AA \vDash \fab{x_1}\phi\br{x_1} $, but we have a term $ t $, and a valuation $ v $ in $ \AA $, with $ v\sbr{\phi\br{t}} = F $. Let
...
\begin{definition}
Let $ \phi $ be an $ \LL $-formula, $ x_i $ a variable, and $ t $ an $ \LL $-term. We say $ t $ is \textbf{free for $ x_i $} in $ \phi $ if there is no variable $ x_j $ in $ t $, such that $ x_i $ has a free occurrence within the scope of a quantifier $ \fab{x_j} $ in $ \phi $.
\end{definition}
...
\end{align*}
For which is $ t $ free for $ x_1 $? $ \phi_1 $.
\end{example*}
...
\subsection{The formal system \texorpdfstring{$ K_\LL $}{KL}}
...
\end{itemize}
\end{itemize}
A \textbf{proof} in $ K_\LL $ is a finite sequence of $ \LL $-formulas, each of which is an axiom, or deduced from previous formulas in the proof using a rule of deduction. A \textbf{theorem} of $ K_\LL $ is the last formula in some proof. Write $ \vdash_{K_\LL} \phi $ if $ \phi $ is a theorem in $ K_\LL $.
\end{definition}
...
\begin{definition}
Suppose $ \Sigma $ is a set of $ \LL $-formulas and $ \psi $ is an $ \LL $-formula. A \textbf{deduction} of $ \psi $ from $ \Sigma $ is a finite sequence of formulas, ending with $ \psi $, each of which is one of
\begin{itemize}
\item an axiom,
...
\begin{theorem}[Deduction theorem]
Suppose $ \LL $ is a first-order language, $ \Sigma $ is a set of $ \LL $-formulas, and $ \phi $ and $ \psi $ are $ \LL $-formulas. Then if $ \Sigma \cup \cbr{\phi} \vdash_{K_\LL} \psi $, then $ \Sigma \vdash_{K_\LL} \impb{\phi}{\psi} $.
\end{theorem}
...
\begin{definition}
A set $ \Sigma $ of $ \LL $-formulas is \textbf{consistent} if there is no formula $ \phi $ with
...
\item Comparing the Lindenbaum lemma in \ref{prop:1.3.8}, there is a consistent set $ \Sigma^* \supseteq \Sigma $ of closed $ \LL $-formulas such that, for every closed $ \LL $-formula $ \psi $, either $ \Sigma^* \vdash_{K_\LL} \psi $ or $ \Sigma^* \vdash_{K_\LL} \notb{\psi} $.
\end{enumerate}
\end{proposition}
...
\begin{notation*}
$ \Sigma \vDash \phi $, if every model of $ \Sigma $ is a model of $ \phi $. Then $ \Sigma \vDash \phi $ implies $ \Sigma \vdash_{K_\LL} \phi $. The converse is soundness.
\end{notation*}
...
by the K1 axiom. So from $ \br{\ref{eq:12}} $ and this fact, and MP, applied $ n $ times, we obtain $ \vdash_{K_\LL} \phi $.
\end{proof}
...
\item If $ \AA $ and $ \BB $ are isomorphic and $ \phi $ is closed, then
...
\label{thm:2.7.4}
If $ \AA $ and $ \BB $ are countable dense linear orders without endpoints, then $ \AA $ and $ \BB $ are isomorphic.
\end{theorem}
...
are consistent. So $ \Sigma \cup \cbr{\phi} $ is consistent. By \ref{thm:2.5.3} and \ref{thm:2.6.6}, it follows that $ \Sigma_1 $ and $ \Sigma_2 $ have countable normal models $ \AA_1 $ and $ \AA_2 $. So $ \AA_1 $ and $ \AA_2 $ are countable dense linear orders without endpoints, and
...
\hfill
\begin{itemize}
\item Depends on the completeness theorem, and the axioms $ \Delta $ for $ \QQ $.
\item Works for some other structures, but can have better algorithms.
\item But, there is no such algorithm for $ \abr{\N; +, \cdot, 0} $. This is \textbf{G\"odel's incompleteness theorem}.
\end{itemize}
\end{note*}
...
\begin{itemize}
\item Extensionality. Sets $ A $ and $ B $ are \textbf{equal} if and only if
...
\begin{definition}
Sets $ A $ and $ B $ are \textbf{equinumerous}, or of the \textbf{same cardinality}, if there is a bijection
$$ f : A \to B. $$
...
$$ f : A \to \N. $$
\item If $ A $ and $ B $ are countable, then
$$ A \times B $$
is countable.
\item If $ A_0, A_1, \dots $ are countable, then
$$ \bigcup_{i \in \N} A_i $$
is countable. The proof uses the axiom of choice.
\end{itemize}
\end{remark}
...
\begin{definition}
For sets $ A $ and $ B $, write $ \abs{A} \le \abs{B} $, or $ A \le B $, if there is an injective function
$$ f : A \to B. $$
\end{definition}
...
\item Is there $ X \subseteq \R $ with $ \abs{\N} < \abs{X} < \abs{\R} $? The \textbf{continuum hypothesis} says no.
\end{itemize}
...
(a) and (b) say that $ B $ is an inductive set. So by $ 1 $, $ \N \subseteq B $. Thus $ \N = B $.
\end{enumerate}
\end{proof}
...
\hfill
\begin{itemize}
\item Say $ \AA_1 $ and $ \AA_2 $ are \textbf{similar}, or \textbf{isomorphic}, if there is a bijection
$$ \alpha : A_1 \to A_2, $$
...
\hfill
\begin{itemize}
\item $ \AA_1 + \AA_2 $ and $ \AA_1 \times \AA_2 $ are linearly ordered sets.
\item If $ \AA_1 $ and $ \AA_2 $ are well orderings, then so are $ \AA_1 + \AA_2 $ and $ \AA_1 \times A_2 $.
\end{itemize}
\end{lemma}
...
\begin{enumerate}
\item $ \AA_1 $ and $ \AA_2 $ are similar,
...
$$ S = \cbr{\beta \mid \beta \ \text{is an ordinal and there is an injective function} \ \beta \to A}. $$
...
\item The compactness theorem, and the compactness theorem for normal models in $ \LL^= $.
\end{itemize}
...
\begin{enumerate}[leftmargin=0.5in, label=Case \arabic*.]
\item $ \phi $ is $ \notb{\psi} $. (Exercise)
\item $ \phi $ is $ \impb{\psi}{\chi} $. (Exercise)
...
\item Adding \textbf{witnesses}. Claim that there is a consistent set of closed $ \LL^+ $-formulas $ \Sigma_\infty \supseteq \Sigma $ such that, for every $ \LL^+ $-formula $ \theta\br{x_i} $ with one free variable, there is some $ b_j $ with
...
\begin{itemize}
\item Base case. $ \phi $ is atomic, that is $ \phi $ is $ R\br{t_1, \dots, t_n} $, for some closed terms $ t_i $, and relation symbol $ R $. Then $ \br{\ref{eq:15}} $ holds by (b) in definition of $ \AA $.
...
& \iff \qquad \Sigma^* \not\vdash_{K_{\LL^+}} \psi & \text{by the induction hypothesis} \ \br{\ref{eq:15}} \\
...
\AA \vDash \notb{\psi\br{b_j}}.
\end{equation}
This contradicts $ \AA \vDash \fab{x_i}\psi $. Take a valuation $ v $ in $ \AA $ with $ v\br{x_i} = \overline{b_j} $, then $ v $ does not satisfy $ \psi $, by $ \br{\ref{eq:16}} $.
\end{itemize}
\end{enumerate}
\end{enumerate}
\end{itemize}
...
$$ \abs{A\sbr{x}} = \abs{\cbr{y \in A \mid y < x}} < \abs{\alpha}. $$
